...
| BL-09 | No input validation anywhere — negative prices, empty strings, and out-of-range decimals are accepted silently | All request records | High | 2026-06-16 | |
| BL-10 | Removing an item from an order does not recalculate `OriginalValue` or `DebitedValue`, leaving order totals stale | `OrderRepository`, `Order.cs` | High | | |
| BL-11 | `TrackingURL` is client-provided at creation — it should be generated by the system when the order reaches `Shipped` | `CreateOrderRequest`, `CreateOrderHandler` | Medium | | |
| BL-12 | `DebitedValue` can exceed `OriginalValue` or be negative with no validation | `CreateOrderRequest` | Medium | | |
| BL-13 | `OrderType` enum has no behaviour — no price modifiers, SLA rules, or discount tiers differ between types | `OrderType.cs`, `CreateOrderHandler` | Medium | | |
| BL-14 | No listing endpoints — no way to list orders by user/status or browse products | `OrdersController`, `ProductsController` | Medium | | |
| BL-15 | Migration is never applied — `Program.cs` resolves the `DbContext` but never calls `Database.Migrate()` | `Program.cs` | Medium | 2026-06-16 | https://github.com/FPDPanda/orders-api-itau/pull/2 |
